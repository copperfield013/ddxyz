...
<style>
	#canteen-delivery-update div.wares-row {
		margin-top		: 0.5em;
		margin-bottom	: 0.5em;
	}
</style>
<c:set var="waresRow">
	<div class="row wares-row">
		...
		<div class="col-lg-4">
			<input type="number" min="0" name="maxCount-0" class="form-control max-count" placeholder="限量" value="\${count} }" />
		</div>
		<div class="col-lg-2">
...
<div id="canteen-delivery-update">
	<div class="page-body">
		<div class="row">
			<div class="col-lg-12">
				<form class="form-horizontal" action="admin/canteen/delivery/do_update/${delivery.id }" method="post">
					<div class="form-group">
						...
						<div class="col-lg-3">
							<input type="text" class="form-control timeRange" id="orderTimeRange" name="orderTimeRange" readonly="readonly" 
	                			css-width="25em"  css-cursor="text" value='<fmt:formatDate value="${delivery.openTime }" pattern="yyyy-MM-dd HH:mm:ss" />~<fmt:formatDate value="${delivery.closeTime }" pattern="yyyy-MM-dd HH:mm:ss" />' />
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-lg-3">
							<input type="text" class="form-control timeRange" id="deliveryTimeRange" name="deliveryTimeRange" readonly="readonly" 
	                			css-width="25em"  css-cursor="text" value='<fmt:formatDate value="${delivery.timePoint }" pattern="yyyy-MM-dd HH:mm:ss" />~<fmt:formatDate value="${delivery.claimEndTime }" pattern="yyyy-MM-dd HH:mm:ss" />' />
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-2 control-label">分发地点</label>
						<div class="col-lg-3">
							<select id="location" name="locationId" data-value="${delivery.locationId }">
...
<script>
	$(function(){
		seajs.use(['ajax', 'dialog', 'utils'], function(Ajax, Dialog, utils){
			console.log('canteen-delivery-update');
			var nav = $("#canteen-delivery-update");
			...
			var dWaresList = $.parseJSON('${deliveryWaresItemsJson}');
			for(var i in dWaresList){
				var dWares = dWaresList[i];
				var $row = $('#waresRow', nav).tmpl({
					count	: dWares.maxCount
				}).change(refreshDisableWares);
				
				$row.append('<input type="hidden" class="deliveryWaresId" value="' + dWares.deliveryWaresId + '" />')
					.find('.wares').attr('disabled', 'disabled').val(dWares.waresId)
					.end().find('.max-count').val(dWares.maxCount)
					.end().find('.dwares-row-del').remove();
				$row.appendTo($('#wares-row-container', nav));
			}
			refreshDisableWares();
			...
			function getDeliveryWaresList(){
				...
				var waresCount = $('.wares-row', nav).each(function(i){
					...
					var $wares = $row.find('.wares').attr('name', 'waresId-' + i),
						...
						$dwId = $row.find('.deliveryWaresId').attr('name', 'deliveryWaresId-' + i);
					var waresId = $wares.val();
					if(waresId){
						list.push({
							waresId	: waresId,
							maxCount: parseInt($maxCount.val()) || 0,
							deliveryWaresId	: $dwId.val()
						});
					}
				}).length;
